...
| LongMemEval-style (500q) | R@5 = **0.9160** / MRR = **0.8618**（top_k=5）；**top_k=10 → R@5=0.992**；**AnswerAcc（LLM-judge，DeepSeek）= 0.678**（GEN-1 生成侧优化后，原 0.602） | 500 题社区生成集（对齐 LongMemEval-S 六类目结构，非官方标注集）；FTS5 keyword。逐类目（top_k=10）：KU 0.825 / SS-A 0.820 / SS-P 0.467 / SS-U 0.920 / **TR 0.675**（原 0.30，TR 专用提示词+judge 提升 2.25×）/ **MS 0.212**（原 0.113，数据集缺陷天花板）。评测入口 `benchmark/answer_eval.py`（支持 `--categories`），产物 `output/answer_eval_results.json` |
