...
## 大脑距离·第二次自评（EXECUTION 460/461 证据，2026-09-02 深夜；240 轮为最近一次官方自评）

> 评分原则：只按"可复核证据"调分；每维给 rubric 依据；无新证据的维度持平不虚涨。

| 维度 | 240 轮 | **二次自评** | Δ | 依据（可复核） |
|---|---|---|---|---|
| 记忆 | 97% | **97%** | 0 | 官方 oracle 500 题 R@1/3/5/10=1.0（六类，两次复现 EXIT=0）；图谱 3,218/17,845；但 cleaned-S 全量 runner 与 M 档未跑 → 不虚涨 |
| 认知 | 96% | **95%** | -1 | QA 生成 0.560→0.578（routed，KU+7.7pp/SS-P+6.7pp 锁证）；MS 0.391 未破（提示级与 naive 会话聚合均证伪，461）→ 生成侧瓶颈如实反映 |
| 意识 | 72% | **75%** | +3 | 情境判据 6→9、蓝图 82→85、情境持续流（ctx:brain 双写+检索注入）、自我公理 120/120、心智工作空间/预测编码在跑；主观体验与深度行动仍缺 → 只 +3 |
| 感知 | 85% | **90%** | +5 | 语义级视觉实测（qwen2.5vl 读出 UI 文字）、持续感知流 vision 通道入库（2 条实证）、情境流携带感知信号；无自动截屏/传感器流 → 90 封顶 |
