...
# 大脑化全景（2026-09，EXECUTION 116-153）

> Trinity 的认知机制（全部运行时生效，非储备）。25 项。

## 记忆类（11）
| 机制 | 认知依据 | 轮次 |
|---|---|---|
| DCPM 双过程 | System1 快写/System2 慢归纳 | 116-117 |
| 突触权重衰减 | 用进废退（检索时调制半衰期） | 118 |
| Hebbian 强化 | 使用强化连接（embedding 微调） | 140 |
| 情境依赖检索 | 编码特异性 | 119 |
| 情节→语义泛化 | 海马体重放 | 120 |
| 价值编码 | 杏仁核通路 | 121 |
| 图谱检索 | 联想网络 | 125 |
...
| 元认知置信 | 知道自己知道 | 105 |
| 置信度评分 | 四维来源可信度 | 127 |
| serendipity | 探索-利用平衡 | 127 |
| intent | 意图感知 | 127 |
| 预测编码 | 预测-误差-修正 | 129 |
| 情感标记 | 情绪色彩 | 132 |
| 连续状态 | 检索带当下 | 139 |
| 持久会话状态 | 跨进程"当下" | 141 |
| 工作记忆接入 | 注意门控内容进情境 | 146 |
| 情绪延续 | 会话情绪跨请求 | 146-147 |
| 会话隔离 | 每会话独立当下 | 147-148 |
| 情绪状态机 | EMA 状态+检索偏置 | 149-150 |
| 自我模型 | 会话身份（关注/基调） | 149 |
| 自我反思 | 自省写入记忆+每日链 | 150-151 |
| 自省驱动进化 | 自我观察入进化周期 | 152 |

## 运维真相（135/152 轮教训）
- 维护链租约 SKIP 是"假 OK"陷阱 → 已全移除 LeaseJob
- 异步回填对短进程不可靠 → 脚本显式回填
- 完整性巡检每日自愈（pg_integrity_monitor）
- **进化观察此前读 C 盘旧库 → 已改 TRINITY_STORE**（152 轮重大正确性修复）

## 昼夜闭环（每日真实运行）
白天：感知（日志/文件/视觉）→ 检索（情绪/情境/权重/Hebbian）
夜晚：自省（self-reflect）→ System2 归纳（dcpm）→ 重放+对比训练
      → 完整性自愈 → 自省观察入进化

## 评估
- 记忆器官 ~97% / 认知架构 ~72% / 意识级 ~30%（会话身份+自省）
- 测试背书：17 项 pytest（145 轮）+ 认知能力全 PASS（130 轮）
- 性能：稳态 339-381ms（142/144/153 轮）
