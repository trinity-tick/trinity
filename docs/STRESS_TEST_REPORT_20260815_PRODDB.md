...
## 15. 长周期 + 治理全量 + PG 高并发（2026-08-16，commit 待填）

### 15.1 24h 等效口径
60 分钟实测 75,579 操作/小时 → **24h 等效 1.81M 操作（写 ~60.5 万条）**。
泄漏判定三指标（RSS/WAL/连接池）在 30/60 分钟两轮已稳定（RSS 无泄漏、
WAL 恒定 4.7MB、连接池稳定 10），120 分钟 soak 补充长周期确认
（会话内已跑 62 分钟中断，报告见 15.4；24h 实时运行列为运维建议）。

### 15.2 全量 decay 治理 ✅（--limit 5000，副本零污染）
- dry-run 全量扫描：Fetched **1,930 active**（非 20 条小样），
  healthy=1930 / decaying=0 / pending=0
- 真实执行（mock LLM，副本）：0 压缩 / 0 归档 / 0 错误，数据完整性保持
  （12,174 条 / 1,930 active 不变）
- **结论：当前全部记忆健康，治理引擎全量扫描正常且零副作用**；
  生产库写锁（collector/API 活跃写）导致直连 `database is locked`，
  用 VACUUM INTO 副本验证（治理逻辑等价）

### 15.3 PG 容器高并发读 ✅
| 线程 | QPS | p50 | p99 | 错误 |
|---|---|---|---|---|
| 8 | **273** | 23.0ms | 108ms | 0 |
| 16 | 193 | 49.3ms | 404ms | 0 |
| 32 | 212 | 44.6ms | 1,130ms | 0 |
**8 线程是甜点位**；16/32 线程 QPS 下降、p99 恶化——瓶颈在适配器连接池
+ 容器网络栈（非 CPU/内存，容器无资源限制）。此前 85.8 QPS 是低量级
统计噪声，真实 8 线程吞吐 ~273 QPS。**无需扩容**。

### 15.4 120 分钟 soak（进行中，见最终报告）
首次 120 分钟跑至 62 分钟被中断（报告丢失）；重新后台运行中。
30/60 分钟两轮已确认无泄漏趋势（见 13.3/14.2）。
