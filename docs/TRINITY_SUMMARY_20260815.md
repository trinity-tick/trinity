...
| 测试 | **626 passed / 43 skipped / 0 failed**（+B3 治理 9 · B5 加密 20 · A4 跨模态 4 · R2 抽取/时序 10） |
...
| R2 新增 | **写路径 LLM 事实抽取**（TRINITY_LLM_EXTRACT，对齐 Mem0/Zep 写入即抽取）· **edge 级 bi-temporal**（relations valid_from/valid_to + query_relations_at） |
...
- **剩余**：官方基准（HF 阻塞）、SaaS/Console、SDK 生态扩展（LangChain 依赖）、prompt cache 感知。
  MCP v2 已实现。规划完成度见 PLANNING_REVIEW_20260815.md（12/15 ✅）。
  2026 Q3 再对比与剩余空间见 COMPARISON_VS_2026_SOTA_R2.md。
