...
while ($true) {
    ...
    if ((Test-Path $Maintenance) -and $now.Hour -eq 3 -and $now.Minute -lt 10 -and $lastDaily -ne $today) {
        Invoke-Script -Path $Maintenance -ArgsList @("-Tasks", "mirror,decay,tiers,consolidate,dedup,sync,compact,agent-ttl,active-health,backup,observe,value-recalib,perception-bridge,dcpm-consolidate") -Label "maintenance(decay,tiers,sync)" -TimeoutSec 2400
        $lastDaily = $today
    }
