...
2. TRINITY_LLM_EXTRACT_ASYNC=on 时 ingest 即时返回（异步语义）
3. LLM 抽取默认同步语义（兼容测试断言）
"""
...
class TestLLMExtractAsync:
    def test_default_sync_semantics(self, monkeypatch):
        """TRINITY_LLM_EXTRACT=on 默认同步：ingest 返回时 postprocess=done。"""
        ...
        assert r.get("postprocess") == "done", \
            f"默认同步应返回 done，实际 {r.get('postprocess')}"
...
@pytest.mark.skipif(
    not __import__("socket").create_connection(("127.0.0.1", 5432), 2),
    reason="native PG :5432 not available")
