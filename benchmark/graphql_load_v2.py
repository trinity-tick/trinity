"""P1: GraphQL load test — v2 with correct field names."""
...
# Correct field names (Strawberry auto-camelCase):
# SearchResult: { score, memory { memoryId, content }, matchedSegments }
# HealthStatus: { status, version, uptimeSeconds, memoryCount, agentCount, componentStatus }
# Agent: { agentId, name, role, status }
# Diagnostics: { component, health, latencyMs, errorRate }
# Memory: { memoryId, content, status }
QUERIES = [
    ("health", "query { health { status version uptimeSeconds componentStatus } }"),
    ("search", 'query { searchMemories(query: "test", limit: 3) { score memory { memoryId content } } }'),
    ("agents", "query { listAgents(limit: 3) { agentId name status } }"),
    ("diagnostics", "query { diagnostics { component health latencyMs } }"),
    ("getMemory", 'query { getMemory(memoryId: "mem_test_001") { memoryId content status } }'),
]
...
print("Warmup...")
r = run_one(schema, QUERIES[0][0], QUERIES[0][1])
print(f"  warmup: {r['latency_ms']}ms success={r['success']}")
