"""P1: GraphQL load test — lightweight version."""
...
QUERIES = [
    ("health", "query { health { status uptime } }"),
    ("search", 'query { searchMemories(query: "test", limit: 3) { items { memoryId content } } }'),
    ("agents", "query { listAgents(limit: 3) { items { agentId name status } } }"),
    ("diagnostics", "query { diagnostics { componentStatus version } }"),
...
print("Warmup...")
run_one(schema, QUERIES[0][0], QUERIES[0][1])
...
print(f"\nSaved: {jp}")
print(f"{'Label':<12} {'P50':<10} {'P95':<10} {'P99':<10} {'QPS':<10} {'Err':<6}")
for r in all_r:
